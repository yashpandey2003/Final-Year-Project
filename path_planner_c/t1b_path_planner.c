...
// Function to find and print the shortest path using Bellman Fords's algorithm
void bellman_ford(int* i, int* j , int* k, int* graph, uint8_t* dist, uint8_t* parent,  uint8_t* current,uint8_t* edgeExist, uint8_t* temp, int start, int end, uint8_t* path_planned, uint8_t* idx) {
...
*u = 28, *v = 29;
 add_edge(graph, u, v);
    ...
    bellman_ford(i,j,k,graph,dist,parent,current,temp,edgeExist, START_POINT, END_POINT, path_planned,idx);
    // _put_str("######### Chk last #########\n");
